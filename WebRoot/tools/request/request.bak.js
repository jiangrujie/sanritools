  ...
  var silmScrollDefault = {alwaysVisible: true,color:'#333'};
  /**
   * 渲染函数,初始化组件宽高
   */
  function render(){
    var $typeMatch = $('#typeMatch'),
        typeMatchHeight = window.clientHeight - $('#columnInput').outerHeight(true) - $('.left-wrap .button-group').outerHeight(true),
        respondHeight = window.clientHeight - $('#request').outerHeight(true) - $url.outerHeight(true);
    
    $typeMatch.height(typeMatchHeight - 8);   // 减去本身的一个 padding-top
    $response.height(respondHeight - $('#responseOpts').outerHeight());  // 减去 border 的就好了
    $(':checkbox[name=isJsonp]').iCheck({
      checkboxClass: 'icheckbox_square-green',
      radioClass: 'iradio_square-green'
    });
    
    $typeMatch.slimScroll($.extend({},silmScrollDefault,{height:(typeMatchHeight - 8)}));
//    $('#response').slimScroll($.extend({},silmScrollDefault));
